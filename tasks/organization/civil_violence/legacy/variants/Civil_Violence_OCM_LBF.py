...
"""
Civil Violence 模型 - OCM-LBF (Original Context Modeling + LLM Behavior Function)
- 简化的上下文信息（核心决策变量）
- LLM自主推理决策（考虑搬家成本、不确定性等）
- 展示LLM在有限上下文下的推理能力
"""
...
class CivilViolence_OCM_LBF:
    def __init__(self, grid_size, steps, citizen_count, cop_count, legitimacy, 
        ...
        # 配置日志
        if log_file is None:
            log_file = f"./logs/civil_ocm_lbf_seed{seed}.log"
        os.makedirs(os.path.dirname(log_file), exist_ok=True)
        
        self.logger = logging.getLogger(f"CivilViolence_OCM_LBF_{seed}")
        ...
        if self.enable_step_log:
            self.step_log_dir = os.path.join(
                self.output_pdf, 
                f"step_logs_OCM_LBF_{self.width}x{self.height}_seed{seed}"
            )
    ...
    def run_simulation(self):
        """运行OCM-LBF仿真"""
        print(f"[OCM-LBF模式] 开始运行 {self.steps} 步仿真...")
        print(f"[说明] LLM自主推理决策（可选择抗议行为和移动目标位置）")
        
        self.init_grid_model()
        self.llm_agent = LLM_Agent(model=self.model, scenario="Civil_Violence_OCM_LBF")
        
        # 初始状态统计
        self._record_history(0, [])
        
        # 迭代
        pbar = tqdm(range(self.steps), desc="OCM-LBF仿真进度", position=0, leave=True)
        for step in pbar:
            # 1. 市民决策 (LLM自主决策：抗议+移动)
            ...
            pbar.set_description(f"OCM-LBF Step {step+1}/{self.steps} (处理 {len(active_citizens)} 个市民)")
            ...
            for idx, citizen in citizen_pbar:
                ...
                # OCM-LBF: 简化上下文，允许LLM推理 + 自主选择移动位置
                attributes = {
                    ...
                    "adjacent_empty_cells": adjacent_empty[:8]  # 最多8个相邻位置
                }
                ...
                try:
                    ...
                    if not isinstance(decision, dict):
                        raise ValueError(f"LLM返回了无效类型: {type(decision)}, 值: {decision}")
                    
                    # 处理抗议决策
                    action = decision.get('decision', 'quiet').lower()
                    if action == 'protest' or action == 'active':
                        citizen['state'] = CitizenState.ACTIVE
                        self.logger.debug(f"Citizen {citizen['id']} LLM决策: ACTIVE")
                    else:
                        citizen['state'] = CitizenState.QUIET
                        self.logger.debug(f"Citizen {citizen['id']} LLM决策: QUIET")
                    ...
                    # 处理移动决策（LBF特性：LLM可以自主决定移动位置）
                    should_move = decision.get('move', False)
                    ...
                    if should_move and target_pos and adjacent_empty:
                        # 验证目标位置
                        if isinstance(target_pos, (list, tuple)) and len(target_pos) == 2:
                            target_pos = tuple(target_pos)
                            if target_pos in adjacent_empty:
                                # 移动到LLM指定的位置
                                empty_cells.remove(target_pos)
                                empty_cells.append(citizen['pos'])
                                self._update_agent_position(citizen, target_pos)
                                self.logger.info(f"Citizen {citizen['id']} moved to LLM-chosen position {target_pos}")
                                if record:
                                    record['movement']['to'] = citizen['pos']
                                    record['movement']['moved'] = record['movement']['from'] != citizen['pos']
                                    record['movement_reason'] = 'llm_target'
                            else:
                                # LLM指定的位置无效，随机选择
                                self.logger.warning(f"Citizen {citizen['id']} LLM指定的位置 {target_pos} 无效，随机选择")
                                if record:
                                    record['notes'] = (record['notes'] + ' target_unavailable').strip()
                                if random.random() < 0.1:  # 10%概率随机移动
                                    ...
                                    if record:
                                        record['movement']['to'] = citizen['pos']
                                        record['movement']['moved'] = record['movement']['from'] != citizen['pos']
                                        record['movement_reason'] = 'llm_target_invalid_random'
                        else:
                            self.logger.warning(f"Citizen {citizen['id']} LLM返回的target_position格式无效: {target_pos}")
                            if record:
                                record['notes'] = (record['notes'] + ' invalid_target_format').strip()
                    elif should_move and not target_pos and adjacent_empty:
                        # LLM决定移动但未指定位置，随机选择
                        self.logger.warning(f"Citizen {citizen['id']} LLM决定移动但未指定位置")
                        if random.random() < 0.1:
                            ...
                            if record:
                                record['movement']['to'] = citizen['pos']
                                record['movement']['moved'] = record['movement']['from'] != citizen['pos']
                                record['movement_reason'] = 'llm_no_target_random'
                    ...
                    if adjacent_empty and random.random() < 0.1:
                        ...
                        if record:
                            record['movement']['to'] = citizen['pos']
                            record['movement']['moved'] = record['movement']['from'] != citizen['pos']
                            record['movement_reason'] = 'fallback_random'
    ...
    def _move_cops(self, empty_cells):
        """警察随机移动到相邻空位"""
        if not empty_cells:
            return
    ...
    def visualization(self):
        ...
        ax_curve.set_title(f'Citizen State Evolution (OCM-LBF)', 
        ...
        filename = f"{self.output_pdf}Civil_OCM_LBF_{self.width}x{self.height}_seed{self.seed}.png"
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        print(f"[保存] 结果保存至: {filename}")
        
        print(f"\n{'='*60}")
        print(f"[统计] Civil Violence OCM-LBF")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Civil Violence OCM-LBF Model")
