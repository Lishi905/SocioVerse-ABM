...
"""
Civil Violence 模型 - OCM-TBF (Original Context Modeling + Traditional Behavior Function)
- LLM严格模拟传统ABM的规则
- 最小化的上下文信息（仅核心决策变量）
- 用于验证LLM能否准确执行传统规则
"""
...
class CivilViolence_OCM_TBF:
    def __init__(self, grid_size, steps, citizen_count, cop_count, legitimacy, 
        ...
        # 配置日志
        if log_file is None:
            log_file = f"./logs/civil_ocm_tbf_seed{seed}.log"
        os.makedirs(os.path.dirname(log_file), exist_ok=True)
        
        self.logger = logging.getLogger(f"CivilViolence_OCM_TBF_{seed}")
        ...
        if self.enable_step_log:
            self.step_log_dir = os.path.join(
                self.output_pdf, 
                f"step_logs_OCM_TBF_{self.width}x{self.height}_seed{seed}"
            )
    ...
    def run_simulation(self):
        """运行OCM-TBF仿真"""
        print(f"[OCM-TBF模式] 开始运行 {self.steps} 步仿真...")
        print(f"[说明] LLM严格执行传统规则: IF (grievance - risk*arrest_prob) > threshold THEN active ELSE quiet")
        
        self.init_grid_model()
        self.llm_agent = LLM_Agent(model=self.model, scenario="Civil_Violence_OCM_TBF")
        
        # 初始状态统计
        self._record_history(0, [])
        
        # 迭代
        pbar = tqdm(range(self.steps), desc="OCM-TBF仿真进度", position=0, leave=True)
        for step in pbar:
            # 1. 市民决策 (LLM执行传统规则)
            ...
            pbar.set_description(f"OCM-TBF Step {step+1}/{self.steps} (处理 {len(active_citizens)} 个市民)")
            ...
            for idx, citizen in citizen_pbar:
                ...
                try:
                    ...
                    if isinstance(decision, dict):
                        ...
                        if record:
                            record['llm_response'] = decision
                            record['notes'] = f"unexpected_llm_response_type:{type(decision).__name__}"
    ...
    def visualization(self):
        ...
        ax_curve.set_title(f'Citizen State Evolution (OCM-TBF)', 
        ...
        filename = f"{self.output_pdf}Civil_OCM_TBF_{self.width}x{self.height}_seed{self.seed}.png"
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        print(f"[保存] 结果保存至: {filename}")
        
        print(f"\n{'='*60}")
        print(f"[统计] Civil Violence OCM-TBF")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Civil Violence OCM-TBF Model")
    ...
    parser.add_argument("--citizen_count", type=int, default=150, help="市民数量")
    parser.add_argument("--cop_count", type=int, default=30, help="警察数量")
    ...
    parser.add_argument("--out_pdf", type=str, default="./results/Civil/OCM-TBF/", help="输出路径")
