#!/usr/bin/env python3
"""
Social Force Model LLM Demo
LLM-based behavior only - no traditional rule-based fallback
...
class SocialForceLLMDemo:
    """LLM demo with LLM-based behavior only - no fallback"""
    
    def __init__(self, scenario='bidirectional'):
        self.scenario = scenario
        self.mode = 'llm'  # Always LLM mode
        self.paused = True  # Start paused
        self.started = False  # Not started yet
        self.animation = None
        
        # Initialize engine with LLM mode only
        self.engine = SocialForceEngine(scenario=scenario, mode=self.mode)
        
        # Create figure and axis
        self.fig, self.ax = plt.subplots(figsize=(14, 8))
        self.fig.suptitle(f'Social Force Model - LLM ({scenario.title()}) - Ready to Start', fontsize=16)
    ...
    def setup_controls(self):
        ...
        ax_status.axis('off')
        self.status_text = ax_status.text(0.5, 0.5, 'LLM Mode Ready\nClick START to begin', 
    ...
    def start_simulation(self, event):
        ...
        if not self.started:
            ...
            self.fig.suptitle(f'Social Force Model - LLM ({self.scenario.title()}) - Running', fontsize=16)
            self.status_text.set_text('LLM Mode Active\nSimulation Running')
    ...
    def stop_simulation(self):
        ...
        try:
            ...
            export_path = f"{export_dir}/sfm_llm_{self.scenario}.csv"
            self.engine.export_metrics_csv(export_path)
            print(f"[EXPORT] Metrics appended to {export_path}")
            # Print LLM experiment stats
            try:
                stats = self.engine.get_llm_experiment_stats()
                print("[LLM STATS]", {
                    "total_requests": stats.get("total_requests_sent"),
                    "iterations": stats.get("iterations"),
                    "total_actions": stats.get("total_actions"),
                    "actions/iter(mean)": stats.get("actions_per_iteration_mean"),
                    "actions/iter(std)": stats.get("actions_per_iteration_std"),
                    "actions/iter[min,max]": (stats.get("actions_per_iteration_min"), stats.get("actions_per_iteration_max")),
                    "unique_agents": stats.get("unique_llm_agents"),
                })
            except Exception as e:
                print(f"[LLM STATS] Failed to compute stats: {e}")
        except Exception as e:
        ...
        self.fig.suptitle(f'Social Force Model - LLM ({self.scenario.title()}) - Stopped', fontsize=16)
        self.status_text.set_text('LLM Mode Ready\nClick START to begin')
    ...
    def change_scenario(self, label):
        ...
        self.ax.set_xlim(-2, self.engine.length + 2)
        self.ax.set_ylim(-2, self.engine.width + 2)
        self.fig.suptitle(f'Social Force Model - LLM ({label.title()}) - Ready to Start', fontsize=16)
    ...
    def toggle_pause(self, event):
        ...
        if self.paused:
            ...
            self.fig.suptitle(f'Social Force Model - LLM ({self.scenario.title()}) - Paused', fontsize=16)
            self.status_text.set_text('LLM Mode Active\nSimulation Paused')
    ...
    def reset_simulation(self, event):
        ...
        self.engine.pedestrians.clear()
        self.engine.step_count = 0
        self.fig.suptitle(f'Social Force Model - LLM ({self.scenario.title()}) - Ready to Start', fontsize=16)
    ...
    def update(self, frame):
        ...
        try:
            ...
            # LLM error - display error and stop simulation
            self.status_text.set_text(f'LLM Error:\n{str(e)[:50]}...')
        ...
        # Update LLM status
        llm_agents = [p for p in self.engine.pedestrians if p.mode == 'llm']
        if llm_agents:
            self.status_text.set_text(f'LLM Mode Active\n{len(llm_agents)} LLM Agents\nNo Fallback')
            self.status_text.set_bbox(dict(boxstyle="round,pad=0.3", facecolor="lightgreen"))
...
def main():
    ...
    print("Social Force Model LLM Demo")
    print("Controls:")
    ...
    print("- LLM-based behavior only - no fallback mechanisms")
    print("- Simulation will stop on LLM errors")
    
    demo = SocialForceLLMDemo(scenario='bidirectional')
