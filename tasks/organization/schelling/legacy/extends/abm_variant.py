...
class SchellingABM:
    ...
    def __init__(self, grid_size, steps, density, minority_pc, homophily, seed, output_pdf,
        ...
        # 配置日志
        if log_file is None:
            log_file = f"./logs/schelling_abm_{self.width}x{self.height}_{self.density}_seed{seed}.log"
        self.logger = setup_logger(log_file, "Schelling_ABM", seed)
    ...
    def run_simulation(self):
        """运行ABM仿真（使用OCC优化）"""
        print(f"[ABM模式] 开始运行 {self.steps} 步仿真...")
        if self.use_occ:
        ...
        # 保存初始状态的 step log
        if self.enable_step_log:
            save_step_log(0, happy_count, self.base.agents, self.base, "ABM", "abm",
                         self.step_log_dir, self.logger)
        
        # 迭代
        print(f"\n开始仿真迭代...")
        pbar = tqdm(range(self.steps), desc="ABM仿真进度",
        ...
        for step in pbar:
            ...
            # 保存 step log (ABM)
            if self.enable_step_log:
                save_step_log(step + 1, happy_count, self.base.agents, self.base, "ABM", "abm",
                             self.step_log_dir, self.logger)
