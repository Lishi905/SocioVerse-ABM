"""DeterministicLLMClient — a pure-function stand-in for the legacy LLMClient (no network).

Its outputs are a deterministic function of the task inputs, so the vendored
``model.step()`` path and this task's env-driven path produce IDENTICAL metric
trajectories on the same seed — the parity proof that the wrapper re-drives the
validated dynamics faithfully, without spending any API budget. Implements exactly
the surface ``SegregationModel.step()`` and the two LLM phases touch.

Ported verbatim from SocioVerse2 ``adapter/fake_llm.py`` so the ABM-repo task and
the SocioVerse2 study share one reference client.
"""
