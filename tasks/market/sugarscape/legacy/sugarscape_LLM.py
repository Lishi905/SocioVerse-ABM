...
# base-URL default was changed (plus the comment on the API-key line). Not maintained (see
# the README in this legacy folder). Comments and messages may be in the original authors'
# language (Chinese).
"""
Sugarscape (G2) with Mesa + LLM-based movement decision
-------------------------
...
# --- API settings -------------------------------------------------
openai.api_key = os.getenv("OPENAI_API_KEY", "")  # read from the environment
openai.api_base = os.getenv("OPENAI_BASE_URL", "https://api.openai.com/v1")

def gpt_generate(prompt, model, max_tokens=64, temperature=0.7, sys_prompt="You are a helpful assistant."):
...
class Citizen(Agent):
    ...
    def locations_in_vision(self) -> List[Tuple[int, int]]:
        """Return all grid locations (pos) visible within vision range in von Neumann directions.
        Includes occupied cells.
        """
            ...
            for r in range(1, self.vision + 1):
                ...
                for obj in self.model.grid.get_cell_list_contents(pos):
                    if isinstance(obj, SugarPatch):
                        sugar_on_patch = obj.sugar
                        break
                locations.append([pos, sugar_on_patch])
        return locations
    
    def step(self) -> None:
        # Using LLM to decide next move
        try:
            candidates = self.locations_in_vision()
            prompt = f"You are a citizen in a sugarscape environment. Your current sugar level is {self.sugar} and your metabolism rate is {self.metabolism}. You can see the following locations within your vision range:\n"
            for loc, sugar in candidates:
                prompt += f"Location {loc} has {sugar} sugar.\n"
            prompt += "Based on this information, which location should you move to in order to maximize your sugar intake while considering your metabolism rate? Please provide the coordinates of the location you choose to move to.\n Format your answer as (x, y) without any other output."
            response = gpt_generate(prompt, model="gpt-4o")
            print("LLM response:", response)
            dest = re.findall(r'\(.*?\)', response)[0]
            dest = eval(dest)
        except Exception as e:
            print("LLM error or invalid response:", e)
